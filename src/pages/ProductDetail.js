...
import { listRelared, productsDetails } from '../components/uiApi';
    ...
    const loadSingleProduct = productId=>{
        productsDetails(productId).then(data=>{
            if(data.error){
                ...
                listRelared(data._id).then(data=>{
                    if(data.error){
                        setError(data.error)
                    }else {
...
<div className='card shadow-lg mb-3 mt-4 offset-md-3' style={{maxWidth:'800px'}}>
    <div className='row'>
        <div className='col-md-6 my-3 p-3'>
            <img src={`http://localhost:5000/${product.product_image}`} className="img-fluid" alt={product.product_name}/>
        </div>
